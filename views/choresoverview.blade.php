...
<div class="row">
	<div class="col">
		...
		<p id="info-assigned-to-me-chores" data-user-filter="xx{{ getenv("GROCY_USER_ID") }}xx" class="btn btn-lg btn-secondary user-filter-button responsive-button"></p>
		@endif
	</div>
</div>
...
<div class="row mt-3">
	...
	<div class="col-xs-12 col-md-6 col-xl-3">
		...
		<select class="form-control input-group-filter" id="user-filter">
			...
			<option class="@if($user->id == getenv("GROCY_USER_ID")) bg-secondary text-white @endif" data-user-id="{{ $user->id }}" value="xx{{ $user->id }}xx">{{ $user->display_name }}</option>
			@endforeach
		</select>
	</div>
	@endif
